...
/* Bifacial panel texture - classic crystalline silicon: blue cells, white busbars, silver fingers */
function makeBifacialTex(){const c=document.createElement("canvas");c.width=512;c.height=1024;const x=c.getContext("2d");
  // Dark backing between cells
  ...
  for(let r=0;r<cr;r++)for(let cl=0;cl<cc;cl++){
    ...
    // Base cell - deep blue with slight variation per cell
    const v=0.85+Math.random()*0.3;
    x.fillStyle="rgb("+Math.floor(22*v)+","+Math.floor(55*v)+","+Math.floor(95*v)+")";
    x.fillRect(cx,cy,w,h);
    // Polycrystalline grain shimmer (subtle lighter patches)
    ...
    // Silver finger lines (thin vertical conductors)
  ...
  // 3 thick white horizontal busbars across full width
  ...
  // Thin horizontal busbars per cell row
  ...
  // Diamond junction markers at intersections
...
/* First Solar thin film texture - uniform matte black, no visible cells */
function makeFirstSolarTex(){const c=document.createElement("canvas");c.width=512;c.height=1024;const x=c.getContext("2d");
  ...
  // Very subtle vertical striations (CdTe thin film deposition lines)
  ...
  // Subtle dark gradient (slight sheen variation)
  ...
  // Very faint edge border (module frame shadow)
  x.strokeStyle="rgba(40,40,45,0.3)";x.lineWidth=4;x.strokeRect(4,4,504,1016);
  // Tiny manufacturer text area (bottom center, barely visible)
...
function build3D(scene,layout,config){
  ...
  const tR=0.12; // torque tube radius
  ...
  layout.rows.forEach(row=>{
    ...
    // Torque tube runs along X (east-west, parallel to DC cables)
    ...
    // Purlins - rails parallel to torque tube, offset along Z (perpendicular)
    const rOff=mw*0.35;
    ...
    for(let p=0;p<row.posts;p++){
      ...
      // String inverter every 4th post for distributed
    ...
    // Solar panels - long edge (mh) along X, short edge (mw) along Z
  ...
  layout.equip.forEach(eq=>{
    ...
    const cc=info.c;const lb=makeLabel(info.l,"rgba("+(cc>>16&0xff)+","+(cc>>8&0xff)+","+(cc&0xff)+",0.85)",eq.t==="substation"?2.5:big?1.8:1);lb.position.set(ex,info.h+(big?1.5:0.8),ez);scene.add(lb);
  });
...
function Scene3D({layout,config,sp,camRef}){
  ...
  const cam=camRef;
  const ms=useRef({down:false,btn:-1,lx:0,ly:0});
  useEffect(()=>{
    ...
    const oD=e=>{ms.current={down:true,btn:e.button,lx:e.clientX,ly:e.clientY};};
    const oM=e=>{if(!ms.current.down)return;const dx=e.clientX-ms.current.lx,dy=e.clientY-ms.current.ly;ms.current.lx=e.clientX;ms.current.ly=e.clientY;
      if(ms.current.btn===0){cam.current.theta-=dx*0.004;cam.current.phi=Math.max(0.05,Math.min(Math.PI/2-0.02,cam.current.phi-dy*0.004));}
    ...
    const oU=()=>{ms.current.down=false;};const oW=e=>{e.preventDefault();cam.current.dist=Math.max(3,Math.min(400,cam.current.dist*(1+e.deltaY*0.001)));};const oC=e=>e.preventDefault();
    const oTS=e=>{if(e.touches.length===1)ms.current={down:true,btn:0,lx:e.touches[0].clientX,ly:e.touches[0].clientY};};
    const oTM=e=>{if(!ms.current.down||e.touches.length!==1)return;const dx=e.touches[0].clientX-ms.current.lx,dy=e.touches[0].clientY-ms.current.ly;ms.current.lx=e.touches[0].clientX;ms.current.ly=e.touches[0].clientY;cam.current.theta-=dx*0.004;cam.current.phi=Math.max(0.05,Math.min(Math.PI/2-0.02,cam.current.phi-dy*0.004));};
    const oTE=()=>{ms.current.down=false;};
...
function SiteDesigner({config,matched,onBack}){
  ...
  // Track camera angle for compass
    ...
    <div style={{width:"100%",height:"100vh",background:T.bgDeep,fontFamily:T.font,color:T.text,display:"flex",flexDirection:"column",overflow:"hidden"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 16px",borderBottom:"1px solid "+T.border,flexShrink:0,background:T.card}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <span style={{fontSize:11,color:T.textSub}}>{MOD[config.module].name} · {config.inverter==="distributed"?"Distributed":config.inverter==="cluster"?"Cluster":"Central"} · {config.dcCollection==="homeruns"?"Homeruns":config.dcCollection==="harnesses"?"Harness":"Trunk Bus"}</span>
        </div>
        ...
        <div style={{flex:1,position:"relative"}}><Scene3D layout={layout} config={config} sp={sp} camRef={camRef} />
          ...
          <div style={{position:"absolute",top:8,right:8,userSelect:"none"}}>
            <svg width="140" height="140" viewBox="-70 -70 140 140" style={{filter:"drop-shadow(0 4px 12px rgba(0,0,0,0.5))"}}>
              ...
              {/* Rotating group - spins with camera */}
...
export default function App(){const[view,setView]=useState("wizard");const[config,setConfig]=useState(null);const[matched,setMatched]=useState(null);
